...
plex_base_url = "http://192.168.20.2:32400"
plex_api_token = "edsPxGfaBhTFpejV-ai"
overseerr_base_url = "http://192.168.20.21:5055"
overseerr_api_token = (
    "MTczODQ3NjQyNzIxMDZhYmYyZjMxLWJlNTgtNGEzMy05MjZmLTEyNWIxZmI3NTUxNA=="
)
# ================================
...
def overseerr_to_collection(
    ...
    # Find plex library
    sections = (
        plex_ssn.get(f"{p_base_url}/library/sections")
        .json()["MediaContainer"]
        .get("Directory", [])
    )
    ...
    while 1:
        requests: List[dict] = (
            overseerr_ssn.get(
                ...
                params={"filter": "available", "take": 50, "skip": offset},
            )
            .json()
            .get("results", [])
        )
    ...
    collections = (
        plex_ssn.get(f"{p_base_url}/library/sections/{lib['key']}/collections")
        .json()["MediaContainer"]
        .get("Metadata", [])
    )
    ...
    # Create collection
    machine_id = plex_ssn.get(f"{p_base_url}/").json()["MediaContainer"][
        "machineIdentifier"
    ]
...
if __name__ == "__main__":
    ...
    parser = ArgumentParser(
        description="Put all requested movies from overseerr in a collection in plex."
    )
    parser.add_argument(
        "-l",
        "--LibraryName",
        type=str,
        required=True,
        help="Name of target movie library",
    )
    parser.add_argument(
        "-c",
        "--CollectionName",
        type=str,
        default="Overseerr Requests",
        help="Name of collection that movies will be put in",
    )
    # autopep8: on
    ...
    try:
        overseerr_to_collection(
            plex_ssn, overseerr_ssn, args.LibraryName, args.CollectionName
        )
